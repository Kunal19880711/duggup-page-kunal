...
<TimelineContainer>
	<PersonInfo {person} />
	{#each feeds as feed}
		{#if feed.type === FeedType.MonthlyPosts}
			<MonthlyPosts monthlyPosts={feed.monthlyPosts} />
		{:else if feed.type === FeedType.RoleChange}
			<RoleChangePost roleChangePost={feed.roleChangePost} />
		{/if}
	{/each}
</TimelineContainer>
